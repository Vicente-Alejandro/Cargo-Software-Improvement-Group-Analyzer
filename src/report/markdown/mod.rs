...
pub fn render_markdown(res: &AnalysisResult, root_dir: &Path) -> String {
    ...
    out.push_str("# SIG Maintainability Audit Report\n\n> Generated by [cargo-sig](https://github.com/Vicente-Alejandro/Cargo-Software-Improvement-Group-Analyzer) — Local Software Improvement Group static analysis.\n\n");
    let mut ctx = MarkdownCtx { out: &mut out, res, root_dir };
    ctx.render_all();
    out
}
...
impl MarkdownCtx<'_> {
    ...
    fn render_scorecard_rows(&mut self) {
        ...
        let d_stars = self.res.delta.map_or(String::new(), |d| format!(" `[{}]`", crate::history::format_delta_stars(d.delta_stars)));
        let d_code = self.res.delta.map_or(String::new(), |d| format!(" `[{}]`", crate::history::format_delta_stars(d.delta_code_stars)));
        let d_vol = self.res.delta.map_or(String::new(), |d| format!(" `[{}]`", crate::history::format_delta_num(d.delta_loc)));
        self.out.push_str("| Metric | Rating | Details |\n| :--- | :---: | :--- |\n");
        let _ = writeln!(self.out, "| **Final Score** | **{}** | **{}/7 Stars**{} |", star_string(s.stars), s.stars, d_stars);
        let _ = writeln!(self.out, "| Code Health | {} | {}/7 Stars{} |", star_string(s.code_stars), s.code_stars, d_code);
        self.render_coverage_row(s);
        let _ = writeln!(self.out, "| System Volume | {} | {}/7 Stars ({} func LOC){} |\n", star_string(s.volume_stars), s.volume_stars, s.total_loc, d_vol);
    }

    #[rustfmt::skip]
    fn render_coverage_row(&mut self, s: &Score) {
        let d_cov = self.res.delta.and_then(|d| d.delta_cov).map_or(String::new(), |c| format!(" `[{}]`", crate::history::format_delta_pct(c)));
        if let (Some(pct), Some(st)) = (s.cov_pct, s.cov_stars) {
            let _ = writeln!(self.out, "| Test Coverage | {} | {}/7 Stars ({:.1}% churn-weighted){} |", star_string(st), st, pct, d_cov);
        } else {
    ...
    fn render_history(&mut self) {
        ...
        self.out.push_str("## Historical Maintainability Trajectory\n\n");
        self.out.push_str("| Date | Commit | Score | Code Health | Coverage | System Volume | Violations | Duplication |\n| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |\n");
